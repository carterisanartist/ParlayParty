...
export function HostLobby({ socket, roomCode, players, currentPlayer }: HostLobbyProps) {
  ...
  const handleStartRound = () => {
    ...
    socket.emit('host:setGameMode', { gameMode: selectedGameMode });
    
    if (selectedGameMode === 'speed') {
      // Start speed mode
      socket.emit('speed:startRound', { roundNumber: 1 });
    } else if (selectedGameMode === 'team') {
      // Navigate to team setup
      socket.emit('gamePhase:teamSetup');
    } else {
      // Start from queue (first video) for classic mode
          ...
          <div className="text-center">
            ...
            <p className="text-fg-subtle mt-3">
              {selectedGameMode === 'speed' 
                ? 'Start Speed Mode - 10 rapid rounds!' 
