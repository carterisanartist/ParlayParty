...
export function VideoQueue({ socket }: VideoQueueProps) {
  ...
  const [videoType, setVideoType] = useState<'youtube' | 'tiktok' | 'upload' | 'stream'>('youtube');
    ...
    <div className="card-neon p-6 space-y-6">
      ...
      <div className="space-y-4 border-t border-fg-subtle/20 pt-6">
        <div className="flex gap-2">
          {(['youtube', 'tiktok', 'upload', 'stream'] as const).map((type) => (
            <button
              key={type}
              ...
              `}
            >
              {type === 'stream' ? '🔴 LIVE' : type.toUpperCase()}
            </button>
          ))}
        </div>
        ...
        {videoType === 'stream' ? (
          <StreamSelector
            socket={socket}
            onStreamSelected={(type, url, platform) => {
              socket.emit('queue:add', {
                videoType: type,
                videoUrl: url,
                title: `${platform?.toUpperCase() || 'Stream'} - Live`,
