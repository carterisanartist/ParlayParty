  ...
  useEffect(() => {
    ...
    socket.on('video:seek', ({ tVideoSec }) => {
      if (player) {
        ...
        videoRef.current.currentTime = tVideoSec;
      }
    });

    socket.on('host:seek', ({ tVideoSec }) => {
      if (player) {
        player.seekTo(tVideoSec, true);
      } else if (videoRef.current) {
        videoRef.current.currentTime = tVideoSec;
  ...
  return (
    <div className="w-full px-6 py-4 space-y-6">
      ...
      <div className="grid lg:grid-cols-[280px_1fr_320px] gap-6">
        ...
        {(round.videoType === 'twitch' || round.videoType === 'kick') && round.streamUrl && round.streamPlatform && (
          <div className="aspect-video bg-black rounded-xl overflow-hidden">
            <StreamPlayer
              streamUrl={round.streamUrl}
              platform={round.streamPlatform}
          ...
          <div className="grid grid-cols-2 gap-4">
            {(round.videoType !== 'twitch' && round.videoType !== 'kick') ? (
              <button
                onClick={() => {
                  if (player) {
                    if (isPaused) {
              ...
              <div className="btn-neon py-4 text-xl font-display text-center bg-gradient-to-r from-purple-600 to-pink-600">
                🔴 LIVE STREAM
              </div>
            )}
            <button
