...
export default function HostLobby({
  ...
  const [players, setPlayers] = useState<Player[]>([
    { id: "1", name: "Carter", joinedAt: Date.now() - 30000 },
    { id: "2", name: "Trevor", joinedAt: Date.now() - 15000 },
  ]);
  ...
  const addVideo = () => {
    if (newVideoUrl.trim()) {
      const newVideo: VideoQueueItem = {
        ...
        title:
          activeTab === "youtube"
            ? "YouTube Video " + (videoQueue.length + 1)
            : "TikTok Video " + (videoQueue.length + 1),
        addedBy: "Host",
      ...
      // Trigger QR pulse when video added
  ...
  const removeVideo = (id: string) => {
    ...
    setVideoQueue(videoQueue.filter((v) => v.id !== id));
  };
  ...
  const canStartGame =
    players.length >= 2 && videoQueue.length >= 1;

  // Simulate new player joining for QR pulse effect
        ...
        <div className="grid grid-cols-12 gap-6 mb-10">
          ...
          >
            <div className="parlay-panel">
              ...
              {/* Debug: Simulate player join */}
              <motion.button
                onClick={simulatePlayerJoin}
                className="mt-4 text-xs text-white/30 hover:text-white/60 w-full"
                style={{ fontFamily: "Rubik, sans-serif" }}
              >
                + Add Test Player
              </motion.button>
          ...
          >
            <div className="parlay-panel h-full flex flex-col items-center justify-center p-8">
              ...
              <div className="relative mb-10">
                ...
                >
                  <div
                    className="grid grid-cols-8 gap-1 p-6"
                    style={{ opacity: 0 }}
                  >
                    {Array.from({ length: 64 }).map((_, i) => (
                      <motion.div
                        key={i}
                        className={`w-full h-6 ${Math.random() > 0.5 ? "bg-black" : "bg-white"} rounded-sm`}
